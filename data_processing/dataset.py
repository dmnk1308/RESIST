...
def write_npz_case(case, raw_data_folder="data/raw", processed_data_folder="data/processed", resolution=128, electrode_resolution=512):
    ...
    # save to file
    np.savez_compressed(case_dir_processed, signals=signal, targets=target, masks=mask, electrodes=electrode, levels=level)
...
def make_dataset(cases, processed_data_folder="data/processed", resolution=128, electrode_resolution=512, mask_resolution=512, 
    ...
    train_dataset = EITData(train_signals, train_targets, train_masks, train_electrodes, train_levels, train_cases, resolution=resolution, training=True, 
    ...
    val_dataset = EITData(val_signals, val_targets, val_masks, val_electrodes, val_levels, val_cases, resolution=resolution, training=False, no_weights=no_weights, 
                          train_mean=train_signal_mean, train_std=train_signal_std, return_electrodes=return_electrodes)
    test_dataset = EITData(test_signals, test_targets, test_masks, test_electrodes, test_levels, test_cases,resolution=resolution, training=False, no_weights=no_weights, 
                           train_mean=train_signal_mean, train_std=train_signal_std, return_electrodes=return_electrodes)
...
class EITData(Dataset):
    def __init__(self, signals, targets, masks, electrodes, levels, cases, training=True, resolution=128, n_sample_points=1000,
                 no_weights=False, train_mean=0, train_std=1, return_electrodes=True):
