...
def predict_case(model, dataset, case, device='cuda', electrode_level_only=True):
  # load
  points_case, signals_case, electrodes_case, masks_case, targets_case = load_case(case, dataset)
  ...
  return preds_case, targets_case, lung_masks_case, eroded_masks_case, electrodes_case, signals_case, points_case
...
def plot_lung_contour(img, mask, cond_value, ax, show_std=False):
    ...
    if show_std:
        ax.text(center_x, center_y, f'{rmse:.4f}\n({(np.round(std, 4)):.4f})', weight='bold',
                color='white', ha='center', va='top', bbox=dict(facecolor='red', alpha=0.8))
    else:
        if rmse != '-':
            ax.text(center_x, center_y, f'{rmse:.4f}', weight='bold',
            color='white', ha='center', va='top', bbox=dict(facecolor='red', alpha=0.8))
        else:
            ax.text(center_x, center_y, f'{rmse}', weight='bold',
            color='white', ha='center', va='top', bbox=dict(facecolor='red', alpha=0.8))
        

def plot_sirt_contour(tomogram, cond_value, center1=(35, 65), center2=(95, 65), axes=(7, 4), ax=None, show_std=False):
        ...
        cv2.ellipse(mask, center1, axes, angle=90, startAngle=0, endAngle=360, color=255, thickness=-1)

        # Draw the second ellipse
        cv2.ellipse(mask, center2, axes, angle=90, startAngle=0, endAngle=360, color=255, thickness=-1)
        ...
        # Annotate the rectangle with the average pixel value
        if show_std:
            ax.text(center_x, center_y, f'{(np.round(rmse, 4)):.4f}({(np.round(std, 4)):.4f})',weight='bold',
                    color='white', ha='center', va='top', bbox=dict(facecolor='red', alpha=0.8))
        else:
            ax.text(center_x, center_y, f'{(np.round(rmse, 4)):.4f}',weight='bold',
            color='white', ha='center', va='top', bbox=dict(facecolor='red', alpha=0.8))
        return ax

def plot_one_level(targets_case, preds_case, lung_masks_case, cmap, level=1, nres=4, show_cbar=True, save=None, aspect_ratio=1):
    level -= 1
    # filter data 
    ...
    x_location = 0.1
    y_location = 0.7
    fig.text(x_location, y_location, f'Ground Truth', ha='center', va='center', rotation='vertical', fontsize=12)
    fig.text(x_location, y_location - 1 * spacing, f'RESIST', ha='center', va='center', rotation='vertical', fontsize=12)
...
def plot_columns(targets_case, preds_case, lung_masks_case, cmap, nlevel=4, nres=4, save=None, aspect_ratio=1):
    for i in range(nres):
...
def plot_grid(targets_case, preds_case, lung_masks_case, nres=4, nlevel=4, show_cbar=True, cbar_orientation='vertical', 
              save=None, y_horizontal_line=0.01, y_vertical_line=0.25, aspect_ratio=1):
    ncols = nres+1
    nrows = nlevel+1
    # set up figure
    fig, axes = plt.subplots(nrows, ncols, figsize=(int((ncols)*1.8), int(1.8*nrows)-1))
    if show_cbar:
        if cbar_orientation == 'vertical': 
            cbar_ax = fig.add_axes([0.95, 0.15, 0.02, 0.7])  # [left, bottom, width, height] # vertical
        elif cbar_orientation == 'horizontal':
            cbar_ax = fig.add_axes([0.15, 0.95, 0.75, 0.02])  # [left, bottom, width, height]  # horizontal
    ...
    for level in range(nlevel):
        ...
        for res in range(nres):
            cond_value = np.mean(targets_case[res,level][lung_masks_case[res,level]==1])
    ...
    fig.add_artist(plt.Line2D([top_left.x0, bottom_right_of_4x4.x1], 
...
def plot_sirt_comparison(targets_case, preds_case, lung_masks_case, case, cmap, 
                         save=None, show_cbar=True, show_sirt=True, aspect_ratio=1):
    rho = [5,10,15,20]
    ...
    for i,r in enumerate(rho):
        ...
        # SIRT
        if show_sirt:
            if os.path.exists('data/raw/'+case+'/tomograms/'+case+'_16adj_rt2_rho_'+str(r)+'_z1_1.egt'):
                sirt_path = 'data/raw/'+case+'/tomograms/'+case+'_16adj_rt2_rho_'+str(r)+'_z1_1.egt'
            else:
                sirt_path = 'data/raw/'+case+'/tomograms/level_1_'+str(r)+'.egt'
            tomogram, axes[2,i] = plot_sirt(sirt_path, 
                                ax=axes[2,i], return_array=True)
    ...
    x_location = 0.08
    y_location = 0.7
    fig.text(x_location, y_location, f'Ground\nTruth', ha='center', va='center', rotation='vertical', fontsize=12)
    fig.text(x_location, y_location - 1 * spacing, f'RESIST', ha='center', va='center', rotation='vertical', fontsize=12)
    fig.text(x_location, y_location - 2.4 * spacing, f'SIRT', ha='center', va='center', rotation='vertical', fontsize=12)
