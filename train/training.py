...
def training(model, train_dataset, val_dataset, epochs, batch_size_train, lr, device, loss_lung_multiplier=1, batch_size_val=2, point_levels_3d=9, model_3d=False, downsample_factor_val=2, output_dir=None):
    ...
    for epoch in pbar:
        ...
        if epoch % 1 == 0: # and epoch!=0:
            epoch_loss_val, epoch_lung_loss_val, epoch_boosted_loss_val = validation_step(model, val_dataloader, loss, device, cmap, 
                                                                point_levels_3d=point_levels_3d, loss_lung_multiplier=loss_lung_multiplier, model_3d=model_3d)
        ...
        wandb.log({"train_loss": epoch_loss_train, "train_lung_loss": epoch_lung_loss_train, "val_loss": epoch_loss_val,"val_lung_loss": epoch_lung_loss_val,"val_boosted_loss": epoch_boosted_loss_val,
                    "best_val_loss": best_val_loss, "best_lung_val_loss": best_lung_val_loss, "best_boosted_val_loss": best_boosted_val_loss})
    return model
...
def validation_step(model, dataloader, loss, device, cmap, point_levels_3d=6, n_examples=4, loss_lung_multiplier=1, model_3d=False):
    model.eval()
    model.to(device)
    epoch_loss = 0
    lung_loss = 0
    ...
    for i, (points, signals, electrodes, mask, target, tissue) in enumerate(dataloader):
        ...
        if num_val >= n_plotting and not plotted:
            ...
            log_heatmaps(targets, preds, n_examples=n_examples, n_levels=4, cmap=cmap, resolution=512)
            plotted = True
