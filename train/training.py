...
def training(model, train_dataset, val_dataset, epochs, batch_size_train, lr, device, loss_lung_multiplier=1, batch_size_val=2, point_levels_3d=9):
    ...
    for epoch in pbar:
        ...
        epoch_loss_val, epoch_lung_loss_val = validation_step(model, val_dataloader, loss, device, cmap, n_plotting=point_levels_3d)
...
def training_step(model, dataloader, optimizer, lr, loss, device, epoch, loss_lung_multiplier):
    ...
    epoch_loss = 0
    lung_loss = 0
    for i, (points, signals, electrodes, mask , targets) in enumerate(dataloader):
        optimizer.zero_grad()
...
def validation_step(model, dataloader, loss, device, cmap, point_levels_3d=6, n_examples=4):
    model.eval()
    model.to(device)
    epoch_loss = 0
    lung_loss = 0
    ...
    for i, (points, signals, electrodes, mask, target) in enumerate(dataloader):
        points = points.reshape(dataloader.batch_size, -1, 512, 512, 3)[:,:,::4,::4,:].reshape(dataloader.batch_size, -1, 3)
        target = target.reshape(dataloader.batch_size, -1, 512, 512)[:,:,::4,::4].reshape(dataloader.batch_size, -1, 1)
        pred = model(signals=signals.to(device), 
                     masks=mask.float().to(device), 
                     electrodes=electrodes.to(device), 
                     xy=points.to(device), 
