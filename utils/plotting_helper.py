...
from data_processing.preprocessing import read_egt, read_get
...
# LOADING
def load_model(path, device="cuda", notebook=False, ckpt_name='model_lung.pt'):
    if not notebook:
        executing_dir = os.path.dirname(os.path.abspath(sys.argv[0]))
        curr_dir = os.path.dirname(os.path.abspath(__file__))
        rel_cfg_dir = os.path.relpath(executing_dir, curr_dir)
        cfg = load_cfg(os.path.join(rel_cfg_dir, path))
        cfg.inference_path = path
    else:
        cfg = load_cfg(path)
    model = hydra.utils.instantiate(cfg.learning.model, model_3d=cfg.data.model_3d)
    model.load_state_dict(
        torch.load(os.path.join(path, ckpt_name), map_location=device)[
            "model_state_dict"
        ],
        strict=True,
    )
...
def pred_postprocess(preds, targets, resolution=512, nlevel=4, nres=4, cond_value=0.2):
    ...
    lung_mask_shrunken = torch.tensor(np.array(
        [
...
# EVALUATION
def evaluate_model(paths, dataset, device='cuda', n_res=4, noise=None, electrode_noise=None, verbose=False):
    mse_total = []
    rmse_total = []
    mse_lung_total = []
    r2_total = []
    ...
    for path in paths:
        ...
        cfg.learning.training.device = device
        metrics = testing(model, dataset, device=cfg.learning.training.device, n_res=n_res,
                                                point_levels_3d=cfg.data.point_levels_3d, wandb_log=False, 
                                                point_chunks=8, electrode_level_only=True, return_metrics=True,
                                                noise=noise, electrode_noise=electrode_noise, verbose=verbose)
        mse, r2, rmse, mse_lung, r2_lung, rmse_lung = metrics.values()
        mse_total.append(mse)
        rmse_total.append(rmse)
        mse_lung_total.append(mse_lung)
        r2_total.append(r2)
        r2_lung_total.append(r2_lung)
        rmse_lung_total.append(rmse_lung)
    
    mse = np.array(mse_total)
    rmse = np.array(rmse_total)
    r2 = np.array(r2_total)
    mse_lung = np.array(mse_lung_total)
    rmse_lung = np.array(rmse_lung_total)
    r2_lung = np.array(r2_lung_total)

    return mse, rmse, r2, mse_lung, rmse_lung, r2_lung
...
def plot_sirt(path, ax=None, threshold=50, return_array=False, plot=True):
    if plot:
        if ax is None:
            fig, ax = plt.subplots()
